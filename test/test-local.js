...
export function handleSummary(data) {
    ...
    const result = {
        ...
        scoring: {
            breakdown: { false_positive_detections: fp, false_negative_detections: fn,
                         true_positive_detections: tp, true_negative_detections: tn, http_errors: errs },
            failure_rate: r(failureRate * 100, PRECISION) + '%',
            weighted_errors_E: E,
            p99_score: { value: r(p99Score, PRECISION), cut_triggered: p99CutTriggered },
            detection_score: { value: r(detScore, PRECISION),
    ...
    return {
        'test/results-local.json': JSON.stringify(result, null, 2),
